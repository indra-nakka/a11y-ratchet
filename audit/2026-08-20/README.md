...
| ocw.mit.edu | course catalogue | 40/40 | Selected from three proposed candidates (MIT OpenCourseWare, Smithsonian collections, UK National Archives Discovery). Publishes an institutional accessibility statement at `accessibility.mit.edu` (HTTP 200, checked before scanning). **29 of the 40 crawled URLs are `/search/` with a different query string each** (`?q=`, `?t=`, `?l=` facets — 29 distinct query strings, confirmed by listing `report.pages`), not 29 independent pages. Query params are preserved by design (`02 §5`: `?page=2` is a different page, and `?q=`/`?t=`/`?l=` aren't in the strip-list), which is correct in general but means a faceted search UI can dominate a page-count budget — 11 of the 40 pages are genuinely distinct non-search URLs. |
