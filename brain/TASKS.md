...
| T40 | Build the churn-resistance demo (wholesale demo-site refactor, same defects), report the diff raw before opening anything | agent | external review #4 | done | branch `demo/churn-resistance` (pushed, no PR, historical) — did NOT come out clean at the time; the gap it found is fixed now, see B8/T42 |
...
| T44 | Trap probe: try Escape and Shift+Tab before a cycle counts as a trap (scoped freeze exception); fixture + test coverage; record against D67 | agent | external review #5 | done | commit `bbee8c9`, `docs/DECISIONS.md` D97 - `test/fixtures/pages/08-focus-trap/` extended with `#modal-escapable`, existing test's assertion tightened (was loose enough to pass even with the fix silently broken), confirmed via `git stash` that it fails without the fix. Full suite 361/361 |
| T45 | Write-only assessment: can axe.ts/focusPath.ts's duplicated identity extraction be unified, cost, blast radius on golden-pairs.test.ts | agent | external review #6 | done | commits `e4619a0`/`22272b6` - `docs/DECISIONS.md` D98, `BUGS.md` B10, README Roadmap updated to a two-phase item. No src/ changes, as instructed |
| T46 | Answer external review's two questions (task 7) - match.ts's 0.65 threshold intent, checkObscured's 2.4.12 extensibility | agent | external review #7 | done | commit `d0e9828`, `docs/DECISIONS.md` D99 - Q1 answered artifact not deliberate (verified against match.test.ts/D45), Q2 confirmed undocumented (verified by search). No code, as instructed. **All 7 external-review tasks now complete.** |
